...
### **🚀 개선이 필요한 점**
1. **처리량이 초당 66.8건 → 10,000명 테스트를 수행하기엔 부족할 가능성**
   - 초당 66.8건 요청이면, **10,000명 사용자를 1분 안에 처리하기 어려움.**
   - 👉 **목표: 초당 최소 166~167건의 요청을 처리**

2. **다음 시도에서 조정할 점**  
   - **Thread 수 (사용자 수) 증가:** 현재 4,000 → 10,000으로 변경  
   - **서버 부하 테스트 지속 시간 증가:** 현재 60초 → 120초로 변경  
   - **처리량 개선 필요:** 레디스 최적화 또는 병렬 처리 방식 확인

---

### **📝 결론 및 추천 설정**
- 현재 **4,000건 요청을 초당 66.8건 처리**하여 **1분(60초) 내 모두 처리 가능**
- 하지만, **10,000명 요청을 테스트하려면 초당 166건 이상의 요청을 감당해야함**
